...
export class WhisperServiceUltimate {
  ...
  async _loadModel(modelStats, modelId) {
    ...
    try {
      ...
      const speedBoost = modelStats.speed_multiplier ? `${modelStats.speed_multiplier}x faster` : "";
      ...
      await modelShareService.shareModel(modelStats.transformers_id, new Blob([`model:${modelId}`]));
      ...
      if (options.useVAD !== false) {
        try {
          ...
          if (vadResult.processedAudio.length > 0) {
            ...
            console.log(`🎤 VAD: Removed ${vadStats.reduction}% silence (${vadStats.segments} speech segments)`);
          }
        } catch (vadError) {
