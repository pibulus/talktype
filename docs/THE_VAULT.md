...
- **Device trust boundary**: TalkType stores the `vault_hash` locally as a passport/sync identifier, not the raw supporter code or AES key. It is still sensitive metadata because it identifies a Vault object and can help offline guessing if supporter codes are weak. Treat browser local storage as trusted-device storage only; the saved hash expires locally and can be regenerated by re-entering the supporter code.
- **Handshake**:
