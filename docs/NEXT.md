...
- Supporter checkout, membership card, Passport QR/link import, automatic encrypted text backup, manual Vault restore, and optional encrypted audio history landed.
...
- Vault has a clear product line: automatic text backup when configured, manual restore/handoff, audio backup opt-in.
...
- End-to-end Vault smoke test with the Pi drop-zone, QRBuddy, automatic text backup, Passport restore, and encrypted audio restore.
...
3. Run a supporter/Vault pass: Square checkout, card QR, automatic text backup, `/passport` import, Vault restore, history audio playback.
