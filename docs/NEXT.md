...
- Supporter checkout, membership card, Passport QR/link import, automatic encrypted history/recording backup, and manual Vault restore landed.
...
- Vault has a clear product line: automatic history/recording backup when configured, plus manual restore/handoff.
...
- End-to-end Vault smoke test with the Pi drop-zone, QRBuddy, automatic history/recording backup, Passport restore, and encrypted audio restore.
...
3. Run a supporter/Vault pass: Square checkout, card QR, automatic history/recording backup, `/passport` import, Vault restore, history audio playback.
