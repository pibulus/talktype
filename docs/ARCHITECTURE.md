...
4. `standard` uses Deepgram Nova-3 batch transcription. When `CHONK_TRANSCRIBE_URL` is set, free-tier `standard` requests are tried first against the in-house transcriber on the Chonk home server (`chonk-transcriber/`, transcribe.cpp + Parakeet, proxied via `src/lib/server/chonkService.js`), falling back to Deepgram on any failure. Supporters always use Deepgram (diarization/paragraphs are their perk).
